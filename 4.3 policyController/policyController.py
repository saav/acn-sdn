...
Actually, the last byte can be almost anything.
But addresses are of the form 10.switchID.portNumber.x.
...
class TopoSwitch (DHCPD):
  ...
  def send_table (self):
    ...
    for dst in switches_by_dpid.itervalues():
      ...
      if policy:
        p =  _get_path(src, switches_by_id.get(3)) + _get_path(switches_by_id.get(3), dst)
      else:
        p = _get_path(src, dst)
